...
object ProviderActor {
  ...
  // msg pour mettre à jour la référence du ConductorActor
}
...
class ProviderActor(database: ActorRef, var conductor: ActorRef) extends Actor {
  ...
  def receive: Receive = {
    ...
    case Measure(chords) =>
      // recoit une mesure et la transmet au ConductorActor
